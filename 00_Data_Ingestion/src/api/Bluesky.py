...
def export_to_csv(feed_data, filename):
    ...
    for feed_items, feed_url in feed_data:
        ...
        for feed_item in feed_items:
            ...
            try:
                created_at = (
                    parser.parse(getattr(record, "created_at", "")).isoformat() + "Z"
                )
            except:
                created_at = ""
            ...
            row = {
                ...
                "Display Name": getattr(author, "display_name", "")
                or "[No Display Name]",
    ...
    with open(filename, "w", newline="", encoding="utf-8-sig") as csvfile:
        ...
        writer = csv.DictWriter(
            csvfile, fieldnames=fieldnames, quoting=csv.QUOTE_ALL, escapechar="\\"
        )
        writer.writeheader()
        writer.writerows(sorted(rows, key=lambda x: x["Created At"], reverse=True))
    ...
    if all_posts:
        try:
            ...
            utils.upload_to_s3(
                s3,
                local_filename,
                f"social-media/bluesky/{local_filename}",
